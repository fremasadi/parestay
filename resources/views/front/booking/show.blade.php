        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-green-700">Pembayaran telah dikonfirmasi. Silakan hubungi pemilik
                                        kost untuk proses check-in.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-yellow-700">Silakan selesaikan pembayaran untuk mengaktifkan
                                        booking Anda.</p>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-md p-6">
                    ...
                    <div class="flex gap-4 mb-4">
                        @if ($booking->kamar)
                            <div class="flex gap-4 mb-4">
                                @if (!empty($booking->kamar->images) && count($booking->kamar->images) > 0)
                                    <img src="{{ asset('storage/' . $booking->kamar->images[0]) }}"
                                        class="w-24 h-24 object-cover rounded-lg"
                                        alt="Kamar {{ $booking->kamar->nomor_kamar }}">
                                @endif
                                <div class="flex-1">
                                    <h4 class="font-bold text-lg mb-1">Kamar {{ $booking->kamar->nomor_kamar }}</h4>
                                    <p class="text-teal-600 font-semibold mb-2">
                                        Rp
                                        {{ number_format($booking->kamar->harga, 0, ',', '.') }}/{{ $booking->kamar->type_harga }}
                                    </p>

                                    @if ($booking->kamar->luas_kamar)
                                        <p class="text-sm text-gray-600">📏 Luas: {{ $booking->kamar->luas_kamar }}</p>
                                    @endif
                                </div>
                            </div>

                            @if (!empty($booking->kamar->fasilitas))
                                <div class="mt-4 pt-4 border-t">
                                    <p class="text-sm font-semibold text-gray-700 mb-2">Fasilitas Kamar:</p>
                                    <div class="flex flex-wrap gap-2">
                                        @foreach ($booking->kamar->fasilitas as $fasil)
                                            <span class="px-3 py-1 bg-teal-50 text-teal-700 text-xs rounded-full">
                                                {{ $fasil }}
                                            </span>
                                        @endforeach
                                    </div>
                        ...
                        @if ($booking->foto_ktp)
                            <div>
                                ...
                                <img src="{{ asset('storage/' . $booking->foto_ktp) }}"
                                    ...
                                    onclick="window.open(this.src, '_blank')" alt="KTP">
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-xl shadow-md p-6">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between">
                                ...
                                <span
                                    class="px-3 py-1 rounded-full text-sm font-semibold {{ $booking->pembayaran->getStatusBadgeClass() }}">
                            ...
                            @if ($booking->pembayaran->settlement_time)
                                <div class="flex justify-between">
                                    ...
                                    <span
                                        class="font-semibold">{{ $booking->pembayaran->settlement_time->format('d M Y, H:i') }}</span>
                                </div>
                            @endif
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-md p-6 sticky top-24">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between text-gray-600">
                            <span>Harga per {{ $booking->durasi_type ?? 'periode' }}</span>
                            <span>Rp {{ number_format($booking->total_harga / $booking->durasi, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-gray-600">
                            <span>Durasi</span>
                            <span>{{ $booking->durasi }} {{ $booking->durasi_type }}</span>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-md p-6">
                    ...
                    @if ($booking->status === 'aktif')
                        <a href="https://wa.me/{{ $booking->kost->pemilik->no_hp ?? '' }}" target="_blank"
