...
@forelse($kosts as $kost)
    <div class="bg-white rounded-2xl overflow-hidden shadow-lg card-hover">
        ...
        <div class="relative h-64">
            ...
            @endif
            <span
                class="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-semibold text-gray-800">
        ...
        <div class="p-6">
            ...
            {{-- FASILITAS --}}
            @php
                $fasilitas = $kost->kamars->flatMap(fn($k) => $k->fasilitas ?? [])->unique()->take(3);
            @endphp
            ...
            <div class="flex items-center justify-between">
                <div>
                    @if ($kost->kamars_min_harga)
                        <span class="text-2xl font-bold teal-accent">
                            Rp {{ number_format($kost->kamars_min_harga, 0, ',', '.') }}
                        </span>
                        <span class="text-gray-500 text-sm">mulai/bulan</span>
                    @endif
                </div>
                <span class="text-sm text-gray-600">
                    <i class="fas fa-door-open mr-1"></i>
                    {{ $kost->kamars->where('status', 'tersedia')->count() }}
                    /
                    {{ $kost->kamars->count() }} Kamar
                </span>
            </div>
